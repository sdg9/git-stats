...
GitStats.prototype.createWorkbook = function (data, callback) {
  ...
  var workbook = excelbuilder.createWorkbook('./workbook', 'git-report' + moment().format(GitStats.DATA_FILE_NAME_DATE_FORMAT) + '.xlsx')

  // Create a new worksheet with 10 columns and 12 rows
  buildPullRequestStatSheet(workbook, data, this.useColors, this.config);
  buildWeeklyStatsSheet(workbook, data, this.config);
  buildUserStatSheet(workbook, data, this.config);
...
function saveToFile(data) {
  createDirIfNotExist(GitStats.DATA_DIR);
  const file = GitStats.DATA_DIR + moment().format(GitStats.DATA_FILE_NAME_DATE_FORMAT) + '.json'
  ...
  fs.readdir(dirname, function(err, filenames) {
    ...
    filenames.forEach(function(filename) {
      fs.readFile(dirname + filename, 'utf-8', function(err, content) {
        ...
        const allFilesRead = dates.length === filenames.length;
        if (allFilesRead) {
...
GitStats.prototype.getMergedJson = function(callback) {
  let self = this;
  readFilesAsOne(GitStats.DATA_DIR, callback);
  /*(dataArray) => {
    // pushing iteratee argument of unionBy to array so it's last argument applied
    // using apply since my input is an array of arrays rather than manually defining each array
    dataArray.push((o) => o.issue.number);
    let combinedData = _.unionBy.apply(_, dataArray);
    callback(combinedData);
  });*/
}
