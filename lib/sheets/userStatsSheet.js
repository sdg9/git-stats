...
module.exports = function(workbook, data, config = {}) {
  ...
  var sheet = workbook.createSheet('User Stats', maxCols, data.length + 1);
  ...
  for (let key of keys(authors).sort()) {
    ...
    for (let date of sortedWeeksMostRecent) {
      ...
      let commented = 0;
      if (date in author) {
        submitted_pull_request = author[date].submitted_pull_request;
        approve = author[date].approve;
        changes_requested = author[date].changes_requested;
        commented = author[date].commented;
      }
      ...
      if (submitted_pull_request <= colorHighlight.lowPRs) {
        sheet.fill(COL.PR_COUNT + step, row, {type:'solid', fgColor: lightRed})
      } else if (submitted_pull_request >= colorHighlight.highPRs) {
        sheet.fill(COL.PR_COUNT + step, row, {type:'solid', fgColor: lightGreen})
      }
