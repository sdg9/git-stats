...
module.exports = function(workbook, data, useColors = false, config = {}) {
  ...
  sheet.set(COL.REVIEW_AFTER_2, 2, 'Review After 2 Approvals');
  ...
  sheet.width(COL.REVIEW_AFTER_2, 15);
  ...
  for (let key of keys(authors).sort()) {
    ...
    sheet.set(COL.FIRST_TO_REVIEW, row, author.total.first_to_review);
    sheet.set(COL.REVIEW_AFTER_2, row, author.total.review_after_2);
